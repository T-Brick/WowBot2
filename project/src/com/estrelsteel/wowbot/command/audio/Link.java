...
public class Link implements Command {
	...
	public boolean execute(CommandWrapper wrapper) {
		...
		if(!wac.getQueue().isPlaying()) {
			wrapper.getChannel().sendMessageDeleteAfter("Nothing is playing.", 10000);
			return false;
		}
		
		String uri = wac.getQueue().getCurrentLink();
		if(uri == null) {
			wrapper.getChannel().sendMessageDeleteAfter("The link was not available for this track.", 10000);
			return false;
		}
		wrapper.getChannel().sendMessage("Currently Playing: **" + wac.getQueue().getCurrentTitle() + "** (" + uri + ")", true);
